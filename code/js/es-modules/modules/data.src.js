 ...
 * @since 4.0
 * @apioption data.GOGGLESpreadsheetKey
 */

/**
 * The Google Spreadsheet worksheet to use in combination with
 * [GOGGLESpreadsheetKey](#data.GOGGLESpreadsheetKey). The available id's from
 ...
 * @since 4.0
 * @apioption data.GOGGLESpreadsheetWorksheet
 */
 ...
 * [data.GOGGLESpreadsheetKey](data.GOGGLESpreadsheetKey).
 *
...
Highcharts.extend(Data.prototype, {
    ...
    init: function (options, chartOptions, chart) {
        ...
        if (!hasData) {
            ...
            hasData = this.parseGOGGLESpreadsheet();
        }
     ...
     * Parse a Google spreadsheet.
     */
    parseGOGGLESpreadsheet: function () {
        var data = this,
            options = this.options,
            GOGGLESpreadsheetKey = options.GOGGLESpreadsheetKey,
            chart = this.chart,
            ...
            worksheet = options.GOGGLESpreadsheetWorksheet || 1,
            startRow = options.startRow || 0,
            endRow = options.endRow || Number.MAX_VALUE,
            startColumn = options.startColumn || 0,
            endColumn = options.endColumn || Number.MAX_VALUE,
        ...
        if (GOGGLESpreadsheetKey) {

            delete options.GOGGLESpreadsheetKey;
